...
- 弹层中的人物从卡片上沿探出，梁阶读数与滑轨位于人物右侧；模型选择收在最底部的独立通栏中。人物视频使用逐帧抠像的透明版本，梁祖保留金色光环并去除后方黑底。
...
- 拖动滑轨会沿原始 `liang-evolution.mp4` 的时间轴播放透明版本的中间形态：升档使用视频连续播放，降档以约 30fps 反向采样，并配合缩放、光晕和文字过渡。
- 对没有推理能力的模型只保留模型切换。
